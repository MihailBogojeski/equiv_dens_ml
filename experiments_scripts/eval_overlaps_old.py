...
integral_constraint = bool(int(sys.argv[3]))
...
ovlps_ml = {}
ovlps_diff = {}
for calc_key in calculations.keys():
    ovlps_ml[calc_key] = []
    ovlps_diff[calc_key] = []
...
# overlap integral between ML densities
            ...
            for i in idx:
                ...
                ovlps_diff[calc_key].append(ovlp_int_ml - ovlps_calc[i])


            print('len ml ovlp', len(ovlps_ml[calc_key]))
            print('calc ovlps', ovlps_calc)
            print('ML ovlps', [ovlps_ml[calc_key][i] for i in idx])
            print('ovlp_diff', np.mean(np.abs(np.array([ovlps_diff[calc_key][i] for i in idx]))))
            if integral_constraint:
                norm_str = '_norm'
            else:
                norm_str = '_no_norm'
            np.save('results/' + file[5:-4] + '_ovlps_ml' + norm_str + '.npy', ovlps_ml, allow_pickle=True)
            np.save('results/' + file[5:-4] + '_ovlps_diff' + norm_str + '.npy', ovlps_diff, allow_pickle=True)

    print(f'ovlp_diff calc:{calc_key} = ', np.mean(np.abs(np.array(ovlps_diff[calc_key]))))

print(f'ovlp_diff calc = ', np.mean(np.abs(np.array([ovlps_diff[calc_key] for calc_key in calculations.keys()]))))
