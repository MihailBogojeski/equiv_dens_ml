...
class NeuralNetwork(nn.Module):
    def __init__(self,
        ...
        if len(orbitals[0][0]) == 2:
            self.density_orbitals = self.calculate_density_basis()
        else:
            self.density_orbitals = self.orbitals
        print('density orbital basis', self.density_orbitals)
        ...
        self.L_counts, self.L_dict = self.compute_orbital_features_num()
        ...
        self.radial_width = nn.ModuleList([nn.Linear(self.num_features, max(self.L_counts) * self.num_radial_components) for L in range(self.order_max + 2)])
        self.radial_scale = nn.ModuleList([nn.Linear(self.num_features, max(self.L_counts) * self.num_radial_components) for L in range(self.order_max + 2)])
    ...
    def calculate_density_basis(self):
        density_orbitals = [None] * len(self.orbitals)
        for i in range(len(self.orbitals)):
            density_L_count = [0] * (self.order_max + 2)
            density_orbitals[i] = []
            ...
            for j in range(len(self.orbitals[i])):
                ...
                # L = orb[1]
                # density_L_count[L] += 1
                orb1 = self.orbitals[i][j]
                l1 = orb1[1]
                for k in range(j, len(self.orbitals[i])):
                    orb2 = self.orbitals[i][k]
                    l2 = orb2[1]
                    for L in range(abs(l1 - l2), min(l1 + l2, self.order_max + 1) + 1):
                        # print('L', L)
                        density_L_count[L] += 1
            for L, c in enumerate(density_L_count):
                if c == 0:
                    continue
                density_orbitals[i].append((z, c, L))
    ...
    def extract_coefficients(self, sph_fs, rad_width, rad_scale):
        spherical_coeffs = [None] * len(self.density_orbitals)
        radial_width = [None] * len(self.density_orbitals)
        radial_scale = [None] * len(self.density_orbitals)
        # print('len radial width', len(radial_width))
        # print('len rad width', len(rad_width))
        for i in range(len(self.density_orbitals)):
            z = self.density_orbitals[i][0][0]
            spherical_coeffs[i] = {}
            radial_width[i] = {}
            radial_scale[i] = {}
            for orb in self.density_orbitals[i]:
                L = orb[2]
                key = (z, L)
    ...
    def forward(self, R):
        ...
        for L in range(len(self.radial_width)):
            out_width.append(F.relu(self.radial_width[L](fs[0])))
            out_width[L] = out_width[L].view(*out_width[L].shape[:-2], self.num_radial_components, max(self.L_counts))
            out_scale.append(self.radial_scale[L](fs[0]))
            out_scale[L] = out_scale[L].view(*out_scale[L].shape[:-2], self.num_radial_components, max(self.L_counts))
        results = {}
