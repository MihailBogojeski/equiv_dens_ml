...
class PairFeaturesV2(nn.Module):
    ...
    def forward(self, atoms):
        ...
        fj = []
        for L in range(self.order+1):
            i = atoms['idx_i'].view(*(1,)*len(fs[L].shape[:-3]),-1,1,1).repeat(*fs[L].shape[:-3], 1, *fs[L].shape[-2:])
            j = atoms['idx_j'].view(*(1,)*len(fs[L].shape[:-3]),-1,1,1).repeat(*fs[L].shape[:-3], 1, *fs[L].shape[-2:])
            fi.append(torch.gather(fs[L], 1, i))
