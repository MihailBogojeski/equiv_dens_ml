...
class HamiltonianParser:
    ...
    def __init__(self, dbpath, basis_definition, orbital_energies=None,
        ...
        self.atomsdata = spk.data.AtomsData(
            dbpath, required_properties=properties
        )
        self.forces = forces
...
class AimsHamiltonianParser(HamiltonianParser):
    ...
    files = {
        ...
        'outfile': ['output', 'OUTPUT', 'aims.out']  # Since output names are inconsistent
    }
    ...
    def _parse_forces(self, path):
        ...
        if outfile is None:
            raise HamiltonianParserException(
                'No output file for energy found in {:s}'.format(path))
        ...
        with open(outfile, 'r') as of:
            for line in of:
                line = line.strip()
                if line.startswith('Total atomic forces (unitary forces cleaned) [eV/Ang]:'):
                    forces = []
                    read = True
                elif not line:
                    read = False
        ...
        if len(forces) == 0:
            raise HamiltonianParserException('No forces found in {:s}'.format(path))
...
class OrcaHamiltonianParser(HamiltonianParser):
    ...
    def __init__(self, dbpath, basis_definition, orbital_energies=None,
        ...
        super(OrcaHamiltonianParser, self).__init__(dbpath, basis_definition,
                                                    orbital_energies,
                                                    check_convergence,
                                                    min_dist, minimal_basis=minimal_basis,
                                                    forces=forces, energy_offset=energy_offset)
    ...
    def _format_basis(self, parsed):
                ...
                if basis_key is None or line.startswith('New'):
                    ...
                    # Extract basis set information
                        ...
                        if basis_idx not in basis_coefficients[basis_key]:
                            ...
                            basis_coefficients[basis_key][basis_idx].append((coeff, expon))

                        max_basis = max(max_basis, len(basis_coefficients[basis_key][basis_idx]))
        ...
        for elem in basis:
            ...
            for idx, current_l in enumerate(current_ls):
                ...
                m = m_range(current_l)
                current_lmn += list(zip((2 * current_l + 1) * [n[idx]], (2 * current_l + 1) * [current_l], m))
...
class OrcaDataParser(OrcaOutputParser):
    ...
    stops = {
        ...
        'basis': ['ORCA GTO INTEGRAL CALCULATION', 'AUXILIARY BASIS SET INFORMATION', 'Checking for AutoStart:'],
    ...
    formatters = {
        ...
        'mos': OrcaFormatter(2, datatype='vector'),
        'xc': OrcaFormatter(2)
    }

    def __init__(self,
                 #properties=['atoms', 'energy', 'mos', 'xc']):
