...
# move file to node.tmp folder on cluster, makes I/O operations much faster
np_dataset,dens_dataset,tmp_dir = move_data_to_cluster_node(args)
log.info(f"Moving np dataset and density dataset temporally to {tmp_dir}")
...
log.info(f"loading atoms from  {args.np_dataset}...")
...
if args.cube_grid:
    ...
    log.info(f"Grid sampling function {cubical_sampling.__name__}")
else:
    grid_fn = partial(spherical_grid, level=args.spherical_grid_level)
    sampling_fn = partial(spherical_radial_sampling, rotate=True)
    grid_origin = 0
    grid_extent = None
    log.info(f"Grid sampling function {spherical_radial_sampling.__name__}")
...
log.info(f"Seed with {args.split_seed}")
...
# choose optimizer
    ...
    if args.optimizer == 'adam':  # Adam
        ...
        log.info("using AMSGrad optimizer")
        optimizers.append(torch.optim.Adam(parameter_list, lr=args.learning_rate, eps=args.epsilon, betas=(
        ...
        log.info("using Stochastic Gradient Descent optimizer")
...
# create summary writer for tensorboard
    ...
    log.info(f'Total params is {total_params}')
...
log.info(f'loss weights test {loss_weights}')
...
for test_batch_num, data in enumerate(test_data_loader):
    ...
    if args.verbose > 1:
        if 'density' in predictions.keys():
            log.info(f"test density intergal {torch.sum(predictions['density'] * predictions['coord_weights'], dim=1)}")
            log.info(f"true density intergal {torch.sum(data['density'] * data['coord_weights'], dim=1)}")
        if 'energy' in predictions.keys():
            log.info(f"pred energy {predictions['energy'].view((-1, ))}")
            log.info(f"true energy {data['energy'].view((-1, ))}")
...
log.info(f'test errors {test_errors}')
log.info(f"Removing temporally folder {tmp_dir}")
