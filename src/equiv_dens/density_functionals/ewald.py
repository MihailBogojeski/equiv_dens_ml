...
class Ewald(nn.Module):
    ...
    def real_energy(self, grid, pos):
        ...
        charges = torch.tensor(charges).to(pos).repeat(positions.shape[0], 1)
        for i in range(len(self.a_num)):
            dists = torch.cdist(positions, pos[:, i].view((-1, 1, 3))).squeeze(-1)
            index = torch.logical_and(dists < rcut, dists > rtol).to(dists)
            dists_div = dists + (dists == 0).to(dists)
            dists_func = index * charges * torch.erfc(eta_sqrt * dists) / dists_div

            esum += self.a_num[i] * torch.sum(dists_func, dim=1)
        esum /= 2.0

        return esum

    def corr_energy(self, grid):
    ...
    def rec_energy(self, grid, pos):
        rec_grid = grid.get_reciprocal_grid()
        a = torch.exp(-1j * torch.einsum("lijk,bl->bijk", rec_grid.coords, pos[:, 0]))
        strf = a * self.a_num[0]
        for i in np.arange(1, len(self.a_num)):
            a = torch.exp(-1j * torch.einsum("lijk,bl->bijk", rec_grid.coords, pos[:, i]))
            strf += a * self.a_num[i]
        ...
        mask = rec_grid.mask
        energy = torch.sum(
            strf_sq[:, mask] * torch.exp(-gg[mask] / (4.0 * self.eta)) * invgg[mask], dim=1)
