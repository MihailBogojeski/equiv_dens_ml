...
sqrt4pi = np.sqrt(1 / (4 * np.pi)) 

# spherical harmonics of order 0
def Y0(u):
    return sqrt4pi * u.new_ones((*u.shape[:-1], 1))
...
def Y1(x, y, z):
    return sqrt4pi * sqrt3 * torch.cat((y, z, x), dim=-1)
...
def Yl(L, x, y, z):
    ...
    return sqrt4pi * torch.cat(Yl, dim=-1)
