...
from .modules.pair_mixing import PairMixing
...
class NeuralNetwork(nn.Module):
    def __init__(self,
                 ...
                 # number of residual blocks applied to output atomic features for constructing pair features (central atoms)
                 num_residual_pc=1,
                 # number of residual blocks applied to output atomic features for constructing pair features (neighboring atoms)
                 num_residual_pn=1,
                 # number of residual blocks applied to output atomic features for predicting irreps of diagonal blocks (shared)
                 num_residual_ii=1,
                 # number of residual blocks applied to pair features for predicting irreps of off - diagonal blocks (shared)
                 num_residual_ij=1,
                 # number of residual blocks applied to output atomic features for predicting irreps of diagonal blocks (full hamiltonian)
                 num_residual_full_ii=1,
                 # number of residual blocks applied to pair features for predicting irreps of off - diagonal blocks (full hamiltonian)
                 num_residual_full_ij=1,
                 # number of residual blocks applied to output atomic features for predicting irreps of diagonal blocks (core hamiltonian)
                 num_residual_core_ii=1,
                 # number of residual blocks applied to pair features for predicting irreps of off - diagonal blocks (core hamiltonian)
                 num_residual_core_ij=1,
                 # number of residual blocks applied to pair features for predicting irreps of off - diagonal blocks (overlap matrix)
                 num_residual_over_ij=1,
        ...
        occupation = torch.tensor([1 if n < sum(
            Zl) // 2 else 0 for n in range(self.Norb)], dtype=torch.float64).unsqueeze(0)
        ...
        self.register_buffer('ZiZj', Zf[:, idx_i] * Zf[:, idx_j])
        self.register_buffer('Z', Z)  # for gathering embeddings
        # for masking out unoccupied orbitals
        self.register_buffer('occupation', occupation)
        ...
        self.mix_s = PairMixing(self.order, self.order, self.order,
                                self.num_basis_functions, self.num_features, self.clebsch_gordan)
        self.mix_ij = PairMixing(self.order, self.order, self.order,
                                 self.num_basis_functions, self.num_features, self.clebsch_gordan)
        ...
        self.residual_pc = ResidualStack(
            self.num_residual_pc, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_pn = ResidualStack(
            self.num_residual_pn, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_ii = ResidualStack(
            self.num_residual_ii, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_ij = ResidualStack(
            self.num_residual_ij, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_full_ii = ResidualStack(
            self.num_residual_full_ii, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_full_ij = ResidualStack(
            self.num_residual_full_ij, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_core_ii = ResidualStack(
            self.num_residual_core_ii, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_core_ij = ResidualStack(
            self.num_residual_core_ij, self.order, self.num_features, self.clebsch_gordan, True, activation)
        self.residual_over_ij = ResidualStack(
            self.num_residual_over_ij, self.order, self.num_features, self.clebsch_gordan, True, activation)
        ...
        number_L = [0 for L in range(2 * order_max + 1)]
        self.irreps_ii = {}
        for i in range(len(self.orbitals)):
        ...
        self.output_full_ii = SphericalLinear(
            self.order, self.num_features, 2 * order_max, max(number_L), self.clebsch_gordan, zero_init=True)
        self.output_core_ii = SphericalLinear(
            self.order, self.num_features, 2 * order_max, max(number_L), self.clebsch_gordan, zero_init=True)
        self.output_over_ii = SphericalLinear(
            self.order, self.num_features, 2 * order_max, max(number_L), self.clebsch_gordan, zero_init=True)
        # diagonal blocks of overlap are constant
        for L in range(self.output_over_ii.order_out + 1):
            # => only bias terms are used
            self.output_over_ii.linear[L].weight.requires_grad = False
        # print('ii', number_L)
        ...
        for z_i, l_i in row:
            ...
            for z_j, l_j in col:
                ...
                for L in range(abs(l_i - l_j), l_i + l_j + 1):
                    ...
                    product = (
                        cg * irreps[idx].unsqueeze(-2).unsqueeze(-2)).sum(-1)

                    # add product to appropriate part of the block
                    blockpart = block.narrow(-2, start_i,
                                             n_i).narrow(-1, start_j, n_j)
                    blockpart += product
    ...
    def forward(self, R):
        ...
        dij, uij = self.calculate_distances_and_directions(
            R, self.idx_i, self.idx_j)
        ...
        for i in range(len(self.orbitals)):  # loop over rows
            ...
            for j in range(len(self.orbitals)):  # loop over columns
                ...
                if self.calculate_full_hamiltonian:
                    full_current_row.append(self.matrix_block(
                        self.orbitals[i], self.orbitals[j], full_irreps, batch_size, j > i, device=R.device, dtype=R.dtype))
                if self.calculate_core_hamiltonian:
                    core_current_row.append(self.matrix_block(
                        self.orbitals[i], self.orbitals[j], core_irreps, batch_size, j > i, device=R.device, dtype=R.dtype))
                if self.calculate_overlap_matrix:
                    over_current_row.append(self.matrix_block(
                        self.orbitals[i], self.orbitals[j], over_irreps, batch_size, j > i, device=R.device, dtype=R.dtype))
            if self.calculate_full_hamiltonian:
        ...
        eye = torch.eye(self.Norb, device=R.device, dtype=R.dtype).unsqueeze(
            0).repeat(batch_size, 1, 1)

        if self.calculate_full_hamiltonian:
            full_hamiltonian = torch.cat(full_matrix_rows, -2)
            full_hamiltonian = full_hamiltonian + \
                full_hamiltonian.transpose(-2, -1)  # symmetrize
        else:
        ...
        if self.calculate_core_hamiltonian:
            ...
            core_hamiltonian = core_hamiltonian + \
                core_hamiltonian.transpose(-2, -1)  # symmetrize
        else:
        ...
        if self.calculate_overlap_matrix:
            ...
            overlap_matrix = overlap_matrix + \
                overlap_matrix.transpose(-2, -1)  # symmetrize
        ...
        if self.calculate_energy:
            ...
            try:
                if self.calculate_overlap_matrix:  # solve Roothan equations FC=SCe
                    eigvals, eigvecs = torch.symeig(
                        overlap_matrix, eigenvectors=True)
                    _, counts = torch.unique_consecutive(
                        eigvals, return_counts=True)
                    if torch.any(counts > 1):
                        degenerate_eigenvalues = True  # will give NaNs in backward pass
                    eps = 1e-8 * torch.ones_like(eigvals)
                    # small eigenvalues mean basis set has linear dependencies
                    eigvals = torch.where(eigvals > 1e-8, eigvals, eps)
                    # transformation matrix for orthogonal basis
                    X = eigvecs / torch.sqrt(eigvals).unsqueeze(-2)
                    Xt = X.transpose(-1, -2)
                    Fs = torch.bmm(torch.bmm(Xt, full_hamiltonian), X)
                    orbital_energies, orbital_coefficients = torch.symeig(
                        Fs, eigenvectors=True)
                    _, counts = torch.unique_consecutive(
                        orbital_energies, return_counts=True)
                    if torch.any(counts > 1):  # will give NaNs in backward pass
                        degenerate_eigenvalues = True  # will give NaNs in backward pass
                    # transform coefficients back to original basis
                    orbital_coefficients = torch.bmm(X, orbital_coefficients)
                else:  # solve simpler Roothan equations FC=Ce (S is identity)
                    orbital_energies, orbital_coefficients = torch.symeig(
                        full_hamiltonian, eigenvectors=True)
            ...
            if symeig_success:
                ...
                hii = torch.bmm(torch.bmm(
                    orbital_coefficients.transpose(- 1, -2), core_hamiltonian), orbital_coefficients)
                # calculate nucleus - nucleus repulsion
                enn = torch.sum(0.5 * self.ZiZj / dij.squeeze(- 1),
                                dim=-1, keepdim=True)
                ...
                energy = enn + \
                    torch.sum(e0 * self.occupation, dim=-1, keepdim=True)
            else:
                ...
                energy = torch.zeros((batch_size, 1), device=R.device,
                                     dtype=R.dtype, requires_grad=self.calculate_forces)
        else:
            orbital_energies = torch.zeros_like(
                torch.diagonal(full_hamiltonian, dim1=-2, dim2=-1))
            orbital_coefficients = torch.zeros_like(full_hamiltonian)
        ...
        if self.calculate_energy and self.calculate_forces and not degenerate_eigenvalues and symeig_success:
            forces = - \
                torch.autograd.grad(torch.sum(energy), R,
