...
# %%
# pos = samp['batch_positions'][0].numpy()
# atom_types = samp['atom_numbers'][0].numpy()
# atom = []
...
# # print(mol.pack())
# mf = dft.RKS(mol)
# print('mol coordinates', mol.atom_coords('Angstrom'))
# mf.chkfile = False
# mf.xc = 'pbe'
# # mf.max_cycle = 1000
# mf.kernel()
...
print('samp positions', samp['batch_positions'])
print('samp dipole moment', samp['dipole_moment'])
print('charges ml', charges_ml)
print('dipoles ml', dipoles_ml)
print('dipoles ml norm', torch.norm(dipoles_ml, dim=-1))
print('dipoles sum', torch.sum((res['batch_positions']*charges_ml.unsqueeze(-1)) + dipoles_ml, dim=1))
print('dipole', res['dipole_moment'])
...
_, h_charges, h_dipoles = hirshfeld_partitioning(samp['density'], samp['atom_density_split'], samp['batch_positions'],
                                                 samp['batch_atom_numbers'], samp['coords'], samp['coord_weights'])
print('hirshfeld charges', h_charges)
print('hirshfeld dipoles', h_dipoles * utils.to_bohr)
print('hirchfeld dipole norms', torch.linalg.norm(h_dipoles, axis=-1))
print('hirshfeld dipole', torch.sum((samp['batch_positions'] * h_charges.unsqueeze(-1)) + h_dipoles, dim=1))
...
print('dipoles df norm', torch.norm(dipoles_df, dim=-1))
print('dipoles df sum', torch.sum((samp_df['batch_positions'] * charges_df.unsqueeze(-1)) + dipoles_df, dim=1))
print('dipole df', samp_df['dipole_moment'])
...
res_ml_df = orbitals.calc_dipole_moment(res_ml_df)
...
print('dipoles ml_df norm', torch.norm(dipoles_ml_df, dim=-1))
print('dipoles sum', torch.sum((res['batch_positions']*charges_ml_df.unsqueeze(-1)) + dipoles_ml_df, dim=1))
print('dipole', res_ml_df['dipole_moment'])

# # %%
# hf_r3_pyscf = np.array([[   6.6795,     6.7566,     6.6149,     6.1268,     6.4444,     7.6477,     31.7581,     29.9442,       72.6591]]).flatten()
# hf_r3_fhiaims = np.array([[ 6.54340538, 6.62575357, 6.52097579, 6.05313996, 6.36643136, 7.41187418, 31.37283684, 29.64926125,   71.94997372]]).flatten()
#
# print(np.corrcoef(hf_r3_fhiaims, hf_r3_pyscf))
# print(np.corrcoef(hf_r3_fhiaims[:6], hf_r3_pyscf[:6]))
#
# # %%
#
# free_r3_pyscf = np.array([[10.736854318479512, 39.53381294166257, 77.79740432782116]]).flatten()
# free_r3_fhiaims = np.array([[10.31591070, 38.38815825, 76.98066615]]).flatten()
#
# print(np.corrcoef(free_r3_fhiaims, free_r3_pyscf))
#
# # %%
# atom_dict = {1:0, 6:1, 16:2}
# anums = torch.flatten(samp['batch_atom_numbers']).numpy()
# pyscf_volume_ratio = []
# fhiaims_volume_ratio = []
#
# for i in range(len(anums)):
#     pyscf_volume_ratio.append(free_r3_pyscf[atom_dict[anums[i]]] / hf_r3_pyscf[i])
#     fhiaims_volume_ratio.append(free_r3_fhiaims[atom_dict[anums[i]]] / hf_r3_fhiaims[i])
# pyscf_volume_ratio = 1/np.array(pyscf_volume_ratio)
# fhiaims_volume_ratio = 1/np.array(fhiaims_volume_ratio)
# print('pyscf   volume ratio', pyscf_volume_ratio)
# print('fhiaims volume ratio', fhiaims_volume_ratio)
# print('volume ratio corr', np.corrcoef(pyscf_volume_ratio, fhiaims_volume_ratio))
# print('volume ratio rmse', np.sqrt(np.mean((np.array(pyscf_volume_ratio) - np.array(fhiaims_volume_ratio))**2)))
#
# %%
model = model_loader.load_model(args, dataset)
idx = 4
...
def calc_dpm_errors(error_dict, atoms, res, removed_free_atom=False):
    center_of_mass = utils.center_of_mass(atoms['batch_positions'], atoms['batch_atom_numbers'])

    # density_err = torch.abs(atoms['density'] - res['density']) * atoms['coord_weights']

    _, h_charges_ml, h_dipoles_ml = hirshfeld_partitioning(res['density'], atoms['atom_density_split'],
                                                           atoms['batch_positions'], atoms['batch_atom_numbers'],
                                                           atoms['coords'], atoms['coord_weights'])
    density_mae = torch.sum(torch.abs(atoms['density'] - res['density']) * atoms['coord_weights']) / torch.sum(atoms['atom_numbers'])
    dpm_err = torch.norm(atoms['dipole_moment'] - res['dipole_moment'])
    dpm_point_abs_err = density_errors.dipole_pointwise_abs_loss(res['density'], atoms['density'], atoms['coords'], atoms['coord_weights'])
    dpm_point_norm_err = density_errors.dipole_pointwise_int_loss(res['density'], atoms['density'], atoms['coords'], atoms['coord_weights'])
    dpm_point_err_r_norm = torch.abs(torch.sum(((res['density'] - atoms['density']) * atoms['coord_weights']) * torch.norm(atoms['coords'] - center_of_mass, dim=-1), dim=1))
    dpm_mag_err = torch.abs(torch.norm(atoms['dipole_moment']) - torch.norm(res['dipole_moment']))
    dpm_ang_error = (180 / torch.pi) * torch.acos(torch.mean(torch.sum(res['dipole_moment']*atoms['dipole_moment']) /
                                                  (torch.norm(res['dipole_moment']) * torch.norm(atoms['dipole_moment']))))
    sh_dist = torch.norm(atoms['batch_positions'][:, -1] - atoms['batch_positions'][:, 5], dim=-1)
    hc_dist = torch.norm(atoms['batch_positions'][:, 5] - atoms['batch_positions'][:, 6], dim=-1)
    atom_charges, atom_dpm = \
        get_atomwise_metrics(res, model.property_models['density'], removed_free_atom=removed_free_atom)

    error_dict['dens_mae'].append(density_mae.detach().cpu())
    error_dict['dpm_norm'].append(dpm_err.detach().cpu())
    error_dict['dpm_point_abs'].append(dpm_point_abs_err.detach().cpu())
    error_dict['dpm_point_norm'].append(dpm_point_norm_err.detach().cpu())
    error_dict['dpm_r_norm'].append(dpm_point_err_r_norm.detach().cpu())
    error_dict['dpm_ang'].append(dpm_ang_error.detach().cpu())
    error_dict['dpm_mag'].append(dpm_mag_err.detach().cpu())
    error_dict['atom_charges'].append(atom_charges.detach().cpu())
    error_dict['atom_dpm'].append(atom_dpm.detach().cpu())
    error_dict['dist_SH'].append(sh_dist.detach().cpu())
    error_dict['dist_HC'].append(hc_dist.detach().cpu())
    error_dict['hirshfeld_charges'].append(h_charges_ml.detach().cpu())
    error_dict['hirshfeld_dipoles'].append(h_dipoles_ml.detach().cpu())
...
ml_dpm_errors = {'dens_mae': [], 'dpm_norm': [],
                 ...
                 'atom_charges': [], 'atom_dpm': [],
                 'dist_SH': [], 'dist_HC': [],
                 'hirshfeld_charges': [], 'hirshfeld_dipoles': [],
                 }
...
for i in range(100):
    ...
    center_of_mass = utils.center_of_mass(samp['batch_positions'], samp['batch_atom_numbers'])

    _, h_charges, h_dipoles = hirshfeld_partitioning(samp['density'], samp['atom_density_split'],
                                                     samp['batch_positions'], samp['batch_atom_numbers'],
                                                     samp['coords'], samp['coord_weights'])
    true_hirshfeld_charges.append(h_charges)
    true_hirshfeld_dipoles.append(h_dipoles)
    res = model(samp)
    res = orbitals.calc_dipole_moment(res)
    # density_err = torch.abs(samp['density'] - res['density']) * samp['coord_weights']
    calc_dpm_errors(ml_dpm_errors, samp, res, model.remove_atom_density)

    # old_df_dens = samp_df['density']
    # old_df_dpm = samp_df['dipole_moment']
    # df_sph_coeffs = {'spherical_coeffs': samp_df['df_coeffs']}
    # df_coeffs = orbitals.vector_to_coeffs_dict(df_sph_coeffs, dataset.orbital_basis_num, samp_df['batch_atom_numbers'], radial_coeffs=True,
    #                                            radial_basis=dataset.radial_coeffs)
    # samp_df.update(df_coeffs)
    # res_df = model.property_models['density'](samp_df)
    # res_df = orbitals.calc_dipole_moment(res_df)
    # new_df_dens = res_df['density']
    # new_df_dpm = res_df['dipole_moment']
    # print('df density diff', torch.sum(torch.abs(old_df_dens - new_df_dens) * samp_df['coord_weights'], dim=1))
    # print('df dpm diff', torch.norm(old_df_dpm - new_df_dpm, dim=1))
    # calc_dpm_errors(df_dpm_errors, samp, res_df)
    #
    # basis = 'augccpvdz'
    # auxbasis = 'augccpvqzjkfit'
    # ml_df_coeffs, auxmol_ext = orbitals.ml_basis_to_df_coeffs(res, basis, auxbasis,
    ...
    # res_ml_df = model.property_models['density'](res_ml_df)
    # res_ml_df = orbitals.calc_dipole_moment(res_ml_df)
    # new_ml_df_dens = res_ml_df['density']
    # new_ml_df_dpm = res_ml_df['dipole_moment']
    # print('df density diff', torch.sum(torch.abs(old_ml_df_dens - new_ml_df_dens) * samp['coord_weights'], dim=1))
    # print('df dpm diff', torch.norm(old_ml_df_dpm - new_ml_df_dpm, dim=1))
    # calc_dpm_errors(ml_df_dpm_errors, samp, res_ml_df)

# print('ml_dpm_errors', ml_dpm_errors)
# print('df_dpm_errors', df_dpm_errors)
# print('ml_df_dpm_errors', ml_df_dpm_errors)
np.save('results/' + main_args.save_file + '_ml_dpm_errors_every1000.npy', ml_dpm_errors, allow_pickle=True)
# np.save('results/' + main_args.save_file + '_df_dpm_errors_loc65.npy', df_dpm_errors, allow_pickle=True)
# np.save('results/' + main_args.save_file + '_ml_df_dpm_errors_loc65.npy', ml_df_dpm_errors, allow_pickle=True)
# %%
...
hirshfeld_dipoles = [dipoles.detach().cpu().numpy() for dipoles in true_hirshfeld_dipoles]
np.save('results/ethanethiol_hirshfeld_dipoles_loc65.npy', hirshfeld_dipoles, allow_pickle=True)
# %%
ml_d_dpm_errors = np.load('results/' + main_args.save_file + '_ml_dpm_errors_loc65.npy', allow_pickle=True).item()
# ml_df_dpm_errors = np.load('results/' + main_args.save_file + '_ml_df_dpm_errors_loc65.npy', allow_pickle=True).item()
# ml_dpm_errors = np.load('results/ethanethiol_ml_dpm_006_errors_loc65.npy', allow_pickle=True).item()
ml_df_dpm_errors = np.load('results/ethanethiol_ml_df_dpm_006_errors_loc65.npy', allow_pickle=True).item()
df_dpm_errors = np.load('results/ethanethiol_df_dpm_006_errors_loc65.npy', allow_pickle=True).item()
ml_dpm_errors = np.load('results/ethanethiol_all_106_ml_dpm_errors_loc65.npy', allow_pickle=True).item()
hirshfeld_charges = np.load('results/ethanethiol_hirshfeld_charges_loc65.npy', allow_pickle=True)
hirshfeld_dipoles = np.load('results/ethanethiol_hirshfeld_dipoles_loc65.npy', allow_pickle=True)
# %%
...
for key in ml_dpm_errors.keys():
    ...
    for i in range(len(ml_dpm_errors[key])):
        ...
        # ml_d_dpm_errors[key][i] = factor * ml_d_dpm_errors[key][i]
        ...
        if torch.numel(ml_dpm_errors[key][i]) == 1:
            # ml_d_dpm_errors[key][i] = float(ml_d_dpm_errors[key][i])
...
# %%
print('ml_dpm_errors dens_mae', np.mean(ml_dpm_errors['dens_mae']))
print('ml_dpm_errors dpm_norm', np.mean(ml_dpm_errors['dpm_norm']))
print('ml_dpm_errors dpm_r_norm', np.mean(ml_dpm_errors['dpm_r_norm']))
print('ml_dpm_errors dpm_ang', np.nanmean(ml_dpm_errors['dpm_ang']))
print('ml_dpm_errors dpm_mag', np.mean(ml_dpm_errors['dpm_mag']))
...
# turn on latex in matplotlib
plt.rcParams['text.usetex'] = True
for i, key in enumerate(keys):
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_dpm_errors[key], label='ML', color=cmap(0))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, df_dpm_errors[key], label='DF', color=cmap(1))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_df_dpm_errors[key], label='ML-DF', color=cmap(2))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_d_dpm_errors[key], label=r'$\Delta$ ML', color=cmap(4))
    axs[i].set_ylabel(labels[i])
    if i == 0:
        axs[i].set_ylim(0.0004, 0.0013)
...
plt.legend()
# plt.savefig('figures/' + main_args.save_file + '_md_dpm_errors_loc65k.png', dpi=300)
plt.savefig('figures/' + main_args.save_file + '_md__dpm_errors_loc65k.pdf', dpi=300)
plt.show()
# %%
...
for i in range(ml_dpm_errors['atom_charges'][0].shape[1]):
    ...
    charge_i_ml_d = [ml_d_dpm_errors['atom_charges'][j][0, i] for j in range(len(ml_d_dpm_errors['atom_charges']))]
    ...
    axs[axs_idx].plot(np.arange(len(ml_dpm_errors['atom_charges']))/2, charge_i_ml_d, label=r"\Delta ML", color=cmap(4))
...
plt.savefig('figures/' + main_args.save_file + '_md_charges_loc65k.pdf', dpi=300)
# plt.savefig('figures/' + main_args.save_file + '_md_charges_loc65k.png', dpi=300)
plt.show()
# %%
from matplotlib.lines import Line2D
...
print('atom_types shape', atom_types.shape)
cmap = plt.get_cmap("tab10")
ranges = [(-0.1, 0.5), (-1.5, 0.1), (-0.35, 0.6)]
for i in range(ml_dpm_errors['atom_charges'][0].shape[1]):
    axs_idx = axes[atom_types[i]]
    charge_i_ml = [ml_dpm_errors['atom_charges'][j][0, i] for j in range(len(ml_dpm_errors['atom_charges']))]
    charge_i_df = [df_dpm_errors['atom_charges'][j][0, i] for j in range(len(df_dpm_errors['atom_charges']))]
...
plt.legend(custom_lines, ['ML', 'DF', 'ML-DF', 'Hirshfeld'])
plt.savefig('figures/' + main_args.save_file + '_md_h_charges_loc65k.pdf', dpi=300)
plt.show()
# %%
from matplotlib.lines import Line2D
...
for i in range(ml_dpm_errors['atom_dpm'][0].shape[1]):
    ...
    dpm_i_ml_d = [torch.norm(ml_d_dpm_errors['atom_dpm'][j][0, i]) for j in range(len(ml_d_dpm_errors['atom_dpm']))]
    ...
    hirshfeld = [np.linalg.norm(hirshfeld_dipoles[j][0, i]) for j in range(len(hirshfeld_dipoles))]
    ...
    axs[axs_idx].plot(np.arange(len(ml_dpm_errors['atom_dpm']))/2, dpm_i_ml_d, label=r"\Delta ML", color=cmap(4))
    ...
    axs[axs_idx].plot(np.arange(len(ml_df_dpm_errors['atom_dpm']))/2, hirshfeld, label="hirshfeld", color=cmap(3))
...
plt.legend(custom_lines, ['ML', 'DF', 'ML-DF', 'Hirshfeld'])
plt.savefig('figures/' + main_args.save_file + '_md_atom_dpm_loc65k.pdf', dpi=300)
plt.show()
...
print('atom_types shape', atom_types.shape)
cmap = plt.get_cmap("tab10")
ranges = [(-0.05, 1), (-0.05, 2.3), (0, 4.3)]
# ranges = [(0, 1), (0, 2.3), (0, 0.8)]
for i in range(ml_dpm_errors['atom_dpm'][0].shape[1]):
    print(atom_types[i])
    axs_idx = axes[atom_types[i]]
    dpm_i_ml = [torch.norm(ml_dpm_errors['atom_dpm'][j][0, i]) for j in range(len(ml_dpm_errors['atom_dpm']))]
...
plt.legend(custom_lines, ['ML', 'DF', 'ML-DF', 'Hirshfeld'])
plt.savefig('figures/' + main_args.save_file + '_md_h_atom_dpm_loc65k.pdf', dpi=300)
plt.show()
# %%
...
mean_hirshfeld = np.mean([hirshfeld_dipoles[i] for i in range(len(hirshfeld_dipoles))], axis=0).squeeze(0)
cmap = plt.get_cmap("tab10")
for i in range(ml_dpm_errors['atom_dpm'][0].shape[1]):
    print(atom_types[i])
    axs_idx = axes[atom_types[i]]
    ...
    hirshfeld = [np.linalg.norm(hirshfeld_dipoles[j][0, i]) - np.linalg.norm(mean_hirshfeld[i])
                 for j in range(len(hirshfeld_dipoles))]
...
for i in range(ml_dpm_errors['atom_dpm'][0].shape[1]):
    ...
    ang_i_ml_df = [cosine_similarity(ml_df_dpm_errors['atom_dpm'][j][0, i], mean_dpm_ml_df[i])
                   for j in range(len(ml_df_dpm_errors['atom_dpm']))]
    ang_hirshfeld = [cosine_similarity(torch.tensor(hirshfeld_dipoles[j][0, i]), torch.tensor(mean_hirshfeld[i]))
                     for j in range(len(hirshfeld_dipoles))]
    ...
    axs[axs_idx].plot(np.arange(len(ml_df_dpm_errors['atom_dpm']))/2, ang_hirshfeld, label="Hirshfeld", color=cmap(3))
...
plt.legend(custom_lines, ['ML', 'DF', 'ML-DF', 'Hirshfeld'])
plt.savefig('figures/' + main_args.save_file + '_md_atom_dpm_dev_loc65k.pdf', dpi=300)
plt.show()
...
for i in range(ml_dpm_errors['hirshfeld_charges'][0].shape[1]):
    axs_idx = axes[atom_types[i]]
    charge_i_ml = [np.abs(ml_dpm_errors['hirshfeld_charges'][j][0, i]) for j in range(len(ml_dpm_errors['hirshfeld_charges']))]
    charge_i_df = [np.abs(df_dpm_errors['hirshfeld_charges'][j][0, i]) for j in range(len(df_dpm_errors['hirshfeld_charges']))]
    charge_i_ml_df = [np.abs(ml_df_dpm_errors['hirshfeld_charges'][j][0, i]) for j in range(len(ml_df_dpm_errors['hirshfeld_charges']))]
...
plt.savefig('figures/' + main_args.save_file + '_md_hirshfeld_charges_loc65k.png', dpi=300)
plt.show()
# %%
ml_even_dpm_errors = np.load('results/ethanethiol_all_001_SH_even_ml_dpm_errors_loc65.npy', allow_pickle=True).item()
# ml_df_dpm_errors = np.load('results/' + main_args.save_file + '_ml_df_dpm_errors_loc65.npy', allow_pickle=True).item()
# ml_dpm_errors = np.load('results/ethanethiol_ml_dpm_006_errors_loc65.npy', allow_pickle=True).item()
ml_mid_dpm_errors = np.load('results/ethanethiol_all_001_SH_mid_ml_dpm_errors_loc65.npy', allow_pickle=True).item()
ml_tail_dpm_errors = np.load('results/ethanethiol_all_001_SH_tail_ml_dpm_errors_loc65.npy', allow_pickle=True).item()
ml_df_dpm_errors = np.load('results/ethanethiol_ml_df_dpm_006_errors_loc65.npy', allow_pickle=True).item()
df_dpm_errors = np.load('results/ethanethiol_df_dpm_006_errors_loc65.npy', allow_pickle=True).item()
hirshfeld_charges = np.load('results/ethanethiol_hirshfeld_charges_loc65.npy', allow_pickle=True)
hirshfeld_dipoles = np.load('results/ethanethiol_hirshfeld_dipoles_loc65.npy', allow_pickle=True)
...
# print('ML_df_dpm_errors dpm_norm', ml_df_dpm_errors['dpm_norm'])
# %%
for key in ml_even_dpm_errors.keys():
    print('key', key)
    print('len key', len(ml_even_dpm_errors[key]))
    for i in range(len(ml_even_dpm_errors[key])):
        factor = 1
        if key == 'atom_charges':
            continue
        if 'dpm' in key and 'ang' not in key:
        ...
        ml_even_dpm_errors[key][i] = factor * ml_even_dpm_errors[key][i]
        ml_mid_dpm_errors[key][i] = factor * ml_mid_dpm_errors[key][i]
        ml_tail_dpm_errors[key][i] = factor * ml_tail_dpm_errors[key][i]
        ml_df_dpm_errors[key][i] = factor * ml_df_dpm_errors[key][i]
        df_dpm_errors[key][i] = factor * df_dpm_errors[key][i]
        if torch.numel(ml_dpm_errors[key][i]) == 1:
            ml_even_dpm_errors[key][i] = float(ml_even_dpm_errors[key][i])
            ml_mid_dpm_errors[key][i] = float(ml_mid_dpm_errors[key][i])
            ml_tail_dpm_errors[key][i] = float(ml_tail_dpm_errors[key][i])
            df_dpm_errors[key][i] = float(df_dpm_errors[key][i])
            ml_df_dpm_errors[key][i] = float(ml_df_dpm_errors[key][i])
# %%
...
plt.rcParams['text.usetex'] = True
for i, key in enumerate(keys):
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_even_dpm_errors[key], label='unbiased', color=cmap(0))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_mid_dpm_errors[key], label='short', color=cmap(3))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_tail_dpm_errors[key], label='long', color=cmap(4))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, df_dpm_errors[key], label='DF', color=cmap(1))
    axs[i].plot(np.arange(len(ml_dpm_errors[key]))/2, ml_df_dpm_errors[key], label='ML-DF', color=cmap(2))
    axs[i].set_ylabel(labels[i])
    if i == 0:
...
plt.legend()
# plt.savefig('figures/' + main_args.save_file + '_md_dpm_errors_loc65k.png', dpi=300)
plt.savefig('figures/' + main_args.save_file + 'combined_md_dpm_errors_loc65k.pdf', dpi=300)
plt.show()
