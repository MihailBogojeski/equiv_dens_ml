...
dpm_point_norm_err_norm = torch.norm(torch.sum((torch.abs(res['density'] - samp['density']) * samp['coord_weights']).unsqueeze(-1) * (samp['coords'] - center_of_mass), dim=1))
print('density_mae', density_mae)
print('dpm norm error', dpm_err)
print('dpm df pointwise error', dpm_point_err.sum())
print('dpm point norm error', dpm_point_norm_err)
print('dpm point abs error', dpm_point_abs_err)
print('dpm point norm error norm', dpm_point_norm_err_norm)
# %%
...
dpm_df_point_norm_err_norm = torch.norm(torch.sum((torch.abs(samp_df['density'] - samp['density']) * samp['coord_weights']).unsqueeze(-1) * (samp['coords'] - center_of_mass), dim=1))
dens = orbitals.sample_projected_density(samp, samp_df['df_coeffs'],
                                         dataset_df.density_fitting['auxbasis'])
...
print('dpm df point error norm', dpm_df_point_abs_err)
print('dpm df point norm error norm', dpm_df_point_norm_err_norm)
# %%
...
# %%
plt.plot(dcom, dpm_df_point_err.detach().cpu().numpy().flatten()[dist_com_idx], label='DF', marker='.', linestyle='none')
...
for i in range(len(eval_L)):
    dens_df = orbitals.expand_df_density_by_degree(samp_df, eval_L[:i + 1], orbital_basis, orbital_basis_size, auxbasis)
...
# %%
samps = checkpoint['data_split_indices']['test'][:100]
basis = 'augccpvdz'
...
for idx in samps:
    ...
    for i in range(len(eval_L)):
        ...
        dens_ml_df = orbitals.expand_df_density_by_degree(samp_ml_df, eval_L[:i + 1], orbital_basis, orbital_basis_size, auxbasis, auxmol_ext)
...
print(np.std(all_ml_df_dpm_errnorm, axis=0))
# %%
np.save('datasets/h2o_test_samps.npy', samps)
np.save('datasets/h2o_all_ml_dens_mae.npy', all_ml_dens_mae)
np.save('datasets/h2o_all_df_dens_mae.npy', all_df_dens_mae)
np.save('datasets/h2o_all_ml_df_dens_mae.npy', all_ml_df_dens_mae)
np.save('datasets/h2o_all_ml_dpm_errnorm.npy', all_ml_dpm_errnorm)
np.save('datasets/h2o_all_df_dpm_errnorm.npy', all_df_dpm_errnorm)
np.save('datasets/h2o_all_ml_df_dpm_errnorm.npy', all_ml_df_dpm_errnorm)

np.save('datasets/h2o_all_ml_dens_errors.npy', all_ml_dens_errors)
np.save('datasets/h2o_all_df_dens_errors.npy', all_df_dens_errors)
np.save('datasets/h2o_all_ml_df_dens_errors.npy', all_ml_df_dens_errors)
np.save('datasets/h2o_all_ml_dpm_errors.npy', all_ml_dpm_errors)
np.save('datasets/h2o_all_df_dpm_errors.npy', all_df_dpm_errors)
np.save('datasets/h2o_all_ml_df_dpm_errors.npy', all_ml_df_dpm_errors)
# %%
# loading data into same variables
samps = np.load('datasets/test_samps.npy', allow_pickle=True)
all_ml_dens_mae = np.load('datasets/h2o_all_ml_dens_mae.npy', allow_pickle=True)
all_df_dens_mae = np.load('datasets/h2o_all_df_dens_mae.npy', allow_pickle=True)
all_ml_df_dens_mae = np.load('datasets/h2o_all_ml_df_dens_mae.npy', allow_pickle=True)
all_ml_dpm_errnorm = np.load('datasets/h2o_all_ml_dpm_errnorm.npy', allow_pickle=True)
all_df_dpm_errnorm = np.load('datasets/h2o_all_df_dpm_errnorm.npy', allow_pickle=True)
all_ml_df_dpm_errnorm = np.load('datasets/h2o_all_ml_df_dpm_errnorm.npy', allow_pickle=True)

all_ml_dens_errors = np.load('datasets/h2o_all_ml_dens_errors.npy', allow_pickle=True)
all_df_dens_errors = np.load('datasets/h2o_all_df_dens_errors.npy', allow_pickle=True)
all_ml_df_dens_errors = np.load('datasets/h2o_all_ml_df_dens_errors.npy', allow_pickle=True)
all_ml_dpm_errors = np.load('datasets/h2o_all_ml_dpm_errors.npy', allow_pickle=True)
all_df_dpm_errors = np.load('datasets/h2o_all_df_dpm_errors.npy', allow_pickle=True)
all_ml_df_dpm_errors = np.load('datasets/h2o_all_ml_df_dpm_errors.npy', allow_pickle=True)
# # %%
# dpm_ml_err_norm = torch.norm(torch.sum(torch.tensor(all_ml_dpm_errors), dim=-2), dim=-1)
# print(torch.corrcoef(torch.stack([dpm_ml_err_norm.flatten(), torch.tensor(all_ml_dpm_errnorm).flatten()], dim=0)))
# # recover vector from density errors
# rs = all_ml_dpm_errors / np.expand_dims(all_ml_dens_errors, axis=-1)
# rs[np.isnan(rs)] = 0
# rs = torch.tensor(rs)
#
# dpm_err = torch.norm(torch.sum(torch.tensor(all_ml_dens_errors).unsqueeze(-1) * rs, dim=-2), dim=-1)
# print(torch.corrcoef(torch.stack([dpm_err.flatten(), torch.tensor(all_ml_dpm_errnorm).flatten()], dim=0)))
#
# dpm_ml_norm_err = torch.sum(torch.tensor(all_ml_dens_errors) * torch.norm(rs, dim=-1), dim=-1)
# print(torch.corrcoef(torch.stack([dpm_ml_norm_err.flatten(), torch.tensor(all_ml_dpm_errnorm).flatten()], dim=0)))
#
# # plt.hist(dpm_ml_norm_err[:, -1].flatten().detach().cpu().numpy(), bins=200)
# # plt.show()
# plt.hist(all_ml_dpm_errors[:, -1].flatten(), bins=200)
# plt.show()
# %%
...
axs[0].plot(np.mean(all_ml_dens_mae, axis=0) * 100, label='ML')
axs[0].plot(np.mean(all_df_dens_mae, axis=0) * 100, label='DF')
axs[0].plot(np.mean(all_ml_df_dens_mae, axis=0) * 100, label='ML-DF')
axs[1].plot(4.8 * np.mean(all_ml_dpm_errnorm, axis=0), label='ML')
axs[1].plot(4.8 * np.mean(all_df_dpm_errnorm, axis=0), label='DF')
axs[1].plot(4.8 * np.mean(all_ml_df_dpm_errnorm, axis=0), label='ML-DF')
...
plt.legend()
# plt.savefig('figures/h2o_dens_errors.png', dpi=300)
plt.show()
# # %%
...
# %%
idx = 1
samps = checkpoint['data_split_indices']['test']
...
for i in range(6):
    ...
    print('DF dpm error', np.sum(all_df_dpm_errors[idx, i].flatten()))
    print('ML dpm error', np.sum(all_ml_dpm_errors[idx, i].flatten()))
    print('ML-DF dpm error', np.sum(all_ml_df_dpm_errors[idx, i].flatten()))
...
dens_diff_range = np.max(np.abs(np.concatenate([df_ml_df_dens_diff[idx], df_ml_ml_dens_diff[idx]])))
dpm_diff_range = np.max(np.abs(np.concatenate([df_ml_df_dpm_diff[idx], df_ml_ml_dpm_diff[idx]])))
samp = dataset.get_properties([samps[idx]])
...
# %%
fig, axs = plt.subplots(6, 2, figsize=(10, 10))
dens_diff_range = np.max(np.abs(np.concatenate([df_ml_ml_dens_diff[idx]])))
dpm_diff_range = np.max(np.abs(np.concatenate([df_ml_ml_dpm_diff[idx]])))
for i in range(6):
    print(dcom.shape)
    axs[i, 0].plot(dcom, df_ml_ml_dens_diff[idx, i].flatten(), label='ML', marker='.', linestyle='none')
    axs[i, 0].set_ylim(-dens_diff_range, dens_diff_range)
...
model = model_loader.load_model(args, dataset)
sample = 2
samp = dataset.get_properties([sample])
fig = plt.figure(figsize=(10, 5))
ax1 = fig.add_subplot(121)
dens = samp['density'].detach().cpu().numpy().flatten()
...
res = model(samp)
p_dens = (res['density'].detach().cpu().numpy().flatten()[idx]).reshape(50, 50)
...
# %%
# samps = checkpoint['data_split_indices']['test'][3470:3475]
samps = list(range(5))
basis = 'augccpvdz'
...
for i in range(6):
    ...
    dens2d_df = all_df_dens_errors[sample, i, 0, idx].reshape(50, 50)
    dens2d_ml = all_ml_dens_errors[sample, i, 0, idx].reshape(50, 50)
    dens2d_ml_df = all_ml_df_dens_errors[sample, i, 0, idx].reshape(50, 50)

    vmin = np.min(np.concatenate([dens2d_df, dens2d_ml, dens2d_ml_df]))
    vmax = np.max(np.concatenate([dens2d_df, dens2d_ml, dens2d_ml_df]))
    c1 = axs[i, 0].contourf(x, y, dens2d_df, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 0].set_aspect('equal')
    c2 = axs[i, 1].contourf(x, y, dens2d_ml, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 1].set_aspect('equal')
    c3 = axs[i, 2].contourf(x, y, dens2d_ml_df, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 2].set_aspect('equal')

    fig.colorbar(c3, ax=axs[i, :])


plt.show()

# %%
# save density differences as cube files
sample = 2

samp = dataset.get_properties([2])
for i in range(6):
    ml_diff = all_ml_dens_errors[sample, i]
    fname = 'datasets/resorcinol_ml_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=ml_diff, atoms=samp, fname=fname, cube_size=args.cube_size)

    df_diff = all_df_dens_errors[sample, i]
    fname = 'datasets/resorcinol_df_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=df_diff, atoms=samp, fname=fname, cube_size=args.cube_size)

    ml_df_diff = all_ml_df_dens_errors[sample, i]
    fname = 'datasets/resorcinol_ml_df_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=ml_df_diff, atoms=samp, fname=fname, cube_size=args.cube_size)
#
#     ml_df_ml_diff = ml_diff - ml_df_diff
#     df_ml_diff = ml_diff - df_diff
# %%
# samps = checkpoint['data_split_indices']['test'][3470:3475]
samps = list(range(5))
basis = 'augccpvdz'
...
for idx in samps:
    ...
    for i in range(len(eval_L)):
        dens_df = orbitals.expand_df_density_by_degree(samp_df, [eval_L[i]], orbital_basis, orbital_basis_size, auxbasis)
        res = model.property_models['density'](res, eval_L=[eval_L[i]])
        dpm = orbitals.calc_dipole_moment(res, normalize_density=False, positive_density=False)['dipole_moment']
        ...
        # res_df['spherical_coeffs'] = sph_coeffs['spherical_coeffs']
        # res_df = model.property_models['density'](res_df, eval_L=eval_L[:i + 1])
        dens_ml_df = orbitals.expand_df_density_by_degree(samp_ml_df, [eval_L[i]], orbital_basis, orbital_basis_size, auxbasis, auxmol_ext)
        dpm_ml_df = orbitals.calc_dipole_moment(samp_ml_df, density=dens_ml_df, normalize_density=False, positive_density=False)['dipole_moment']
...
for i in range(6):
    ...
    samp = dataset.get_properties([2])
    res = model(samp)
    print(res['density'].shape)
    ml_dens = model.property_models['density'](res, eval_L=[i])['density'].detach().cpu().numpy()
    ml_df_dens = reconstruct_dens(samp['density'], all_ml_df_dens_errors[sample, i], samp['coord_weights'])
    dens2d_1 = ml_dens[0, idx].reshape(50, 50)
    dens2d_2 = ml_df_dens[0, idx].reshape(50, 50)
    dens2d_3 = (all_ml_dens_errors[sample, i, 0, idx] - all_ml_df_dens_errors[sample, i, 0, idx]).reshape(50, 50)

    vmin = np.min(dens2d_1)
    vmax = np.max(dens2d_1)
    c1 = axs[i, 0].contourf(x, y, dens2d_1, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 0].set_aspect('equal')
    c2 = axs[i, 1].contourf(x, y, dens2d_2, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 1].set_aspect('equal')
    vmin = np.min(dens2d_3)
    vmax = np.max(dens2d_3)
    c3 = axs[i, 2].contourf(x, y, dens2d_3, vmin=vmin, vmax=vmax, levels=100)
    axs[i, 2].set_aspect('equal')
...
for i in range(6):
    ...
    fname = 'datasets/resorcinol_ml_df_ml_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=ml_df_diff - ml_diff, atoms=samp, fname=fname, cube_size=args.cube_size)

    fname = 'datasets/resorcinol_ml_df_df_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=ml_df_diff - df_diff, atoms=samp, fname=fname, cube_size=args.cube_size)

    fname = 'datasets/resorcinol_df_ml_diff_L' + str(i) + '.cube'
    utils.write_cube_from_atoms(density=df_diff - ml_diff, atoms=samp, fname=fname, cube_size=args.cube_size)
